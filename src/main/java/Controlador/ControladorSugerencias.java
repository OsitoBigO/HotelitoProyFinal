...
public class ControladorSugerencias extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (accion != null) {
            ...
            else if(accion.equals("listarMateriales")){
                List<msugerencia> listaMat=new SugerenciasDAO().getList();
                response.setContentType("application/json;charset=UTF-8");
                String listaMaterialesJson= new Gson().toJson(listaMat);
                response.getWriter().println(listaMaterialesJson);
            }
        }
